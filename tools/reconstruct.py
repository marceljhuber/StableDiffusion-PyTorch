...
def reconstruct_image(image_tensor, model, scheduler, vae, train_config, diffusion_config):
    ...
    print(image_tensor.unsqueeze(0).to(device).shape)
    xt = vae.encode(image_tensor.unsqueeze(0).to(device))[0]

    # sample_classes = torch.tensor([0, 1, 2, 3])
    sample_classes = torch.tensor([diffusion_config['class']])
    print('Generating images for {}'.format(list(sample_classes.numpy())))
    cond_input = {
        'class': torch.nn.functional.one_hot(sample_classes, 4).to(device)
    }
    ...
    final_image = torch.clamp(final_image, -1., 1.).detach().cpu()
